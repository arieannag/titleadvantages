...
<body>
  ...
  <div class="grid-container" id="grid-container">
    ...
    <main>
      ...
      <div class="netAdvantageIcons" id="netAdvantageIcons">
        <form action="quickSeller.php" method="POST" name="theform" id="netForm" onsubmit="return validateForm()">
          ...
          <div>
            <label for="street_name">* Sales Price</label>
            <input type="text" size="10" maxlength="30" id="street_name" name="salesPrice">
          </div>

          <div>
            <label for="1stmortgageamt">1st Mort Balance</label>
            <input type="text" size="10" maxlength="20" name="1stmortgageamt" id="1stmortgageamt">
            <!-- <input type="button" id="calcmortgagebutton" value="calculate" onClick="window.open('mortbalestimate.asp', 'newwin', 'width=400, height=460, location=0, menubar=0, resizable=0, scrollbars=1, status=1, titlebar=1, toolbar=1');"> -->

          </div>

          <div>
            <label for="mInterestRate">1st Mort Interest %</label>
            <input type="text" size="10" maxlength="12" name="mInterestRate" id="1stmortgagerate">
          </div>
  ...
  <script>
    function validateForm() {
      ...
      alerts.innerHTML = "";
      var salesPrice = document.forms["theform"]["salesPrice"].value;
      salesPrice = salesPrice.replace("$", "");
      salesPrice = salesPrice.replace(",", "");
      parseInt(salesPrice);
      var firstmortgageamt = document.forms["theform"]["1stmortgageamt"].value;
      firstmortgageamt = firstmortgageamt.replace("$", "");
      firstmortgageamt = firstmortgageamt.replace(",", "");
      parseInt(firstmortgageamt);
      ...
      if (firstmortgageamt == "") {
        firstmortgageamt = 0;
        document.forms["theform"]["1stmortgageamt"].value = 0;
      }
      ...
      if (salesPrice == "" || pzip == "") {
        ...
        return false;
      } else if (!salesPrice.match(regex) || !firstmortgageamt.match(regex) || !mInterestRate.match(regex) || !pzip.match(regex)) {

        if (!salesPrice.match(regex)) {
          alerts.innerHTML += '<p style="color: red;">Sale Price must be a numeric value</p>';
        }
        if (!firstmortgageamt.match(regex)) {
          alerts.innerHTML += '<p style="color: red;">1st Mort Balance must be a numeric value</p>';
        }
