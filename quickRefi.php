...
<body>
  ...
  <div class="grid-container" id="grid-container">
    ...
    <main>
      ...
      <div class="netAdvantageIcons" id="netAdvantageIcons">
        <form action="quickRefi-b.php" method="POST" name="theform" id="netForm" onsubmit="return validateForm()">
          ...
          <div id="alerts"></div>
          <div>
            <label for="autocomplete">Report Name</label>
            <input id="autocomplete" type="text" size="25" maxlength="30" name="reportName"></input>
          </div>

          <div>
            <label for="street_name">* Loan Amount $</label>
            <input type="text" size="10" maxlength="30" id="street_name" name="loanAmount">
          </div>

          <div>
            <label for="street_name">Loan Interest Rate</label>
            <input type="text" size="10" maxlength="30" id="street_name" name="interestRate">
          </div>

          <div>
            <label for="mortBalance">1st Mort Balance $</label>
            <input type="text" size="10" maxlength="20" name="mortBalance" id="mortBalance">
          </div>

          <div>
            <label for="mInterestRate">1st Mort Interest %</label>
            <input type="text" size="10" maxlength="12" name="mInterestRate" id="mInterestRate">
          </div>

          <div>
            <label for="pzip">* Property Zip Code </label>
            <input type="text" size="10" maxlength="12" name="pzip" id="pzip">
          </div>

          <div style="text-align: center;">
            <input type="submit" value=" Continue " id="search" class="button">
            <input type="reset" value=" Cancel - Go Back " onClick="history.back(-1);" class="button">
          </div>
        </form>
  ...
  <script language="JavaScript">
    function settheme() {
      document.bgColor = '#a29c8e';

    }

    function cpon() {
      document.quickprofileform.coverpage.value = 'on';
    }

    function showload() {
      document.all.links.style.display = document.all.links.style.display == 'none' ? '' : 'none';
      document.all.conf.style.display = document.all.conf.style.display == 'none' ? '' : 'none';
    }

    function qpload(apn, fipscode, pcity, ps, pc, pz, pa) {
      document.all.quickprofileform.propapn.value = apn;
      document.all.quickprofileform.fipscode.value = fipscode;
      document.all.quickprofileform.propcity.value = pcity;
      document.all.quickprofileform.propstate.value = ps;
      document.all.quickprofileform.propcounty.value = pc;
      document.all.quickprofileform.propzip.value = pz;
      document.all.quickprofileform.propaddress.value = pa;
      document.all.quickprofileform.submit();
    }

    function subform(place) {
      if (place == "excel") {
        document.downform.action = 'prosearch-excel.asp';
        document.downform.submit();
      }

      if (place == "comm") {
        document.downform.action = 'prosearch-commdelim.asp';
        document.downform.submit();
      }
    ...
    function validateForm() {
      ...
      alerts.innerHTML = "";
      var loanAmount = document.forms["theform"]["loanAmount"].value;
      loanAmount = loanAmount.replace("$", "");
      loanAmount = loanAmount.replace(",", "");
      parseInt(loanAmount);
      var interestRate = document.forms["theform"]["interestRate"].value;
      interestRate = interestRate.replace("%", "");
      var mortBalance = document.forms["theform"]["mortBalance"].value;
      mortBalance = mortBalance.replace("$", "");
      mortBalance = mortBalance.replace(",", "");
      parseInt(mortBalance);
      var mInterestRate = document.forms["theform"]["mInterestRate"].value;
      ...
      if (interestRate == "") {
        interestRate = '0';
        document.forms["theform"]["interestRate"].value = 0;
      }
      if (mortBalance == "") {
        mortBalance = '0';
        document.forms["theform"]["mortBalance"].value = 0;
      }
      if (mInterestRate == "") {
        mInterestRate = '0';
        document.forms["theform"]["mInterestRate"].value = 0;
      }

      if (loanAmount == "" || pzip == "") {
        if (loanAmount == "") {
          alerts.innerHTML += '<p style="color: red;">Please enter a Loan Amount </p>';
        }

        if (pzip == "") {
          alerts.innerHTML += '<p style="color: red;">Please enter a Zip Code </p>';
        }

        console.log("one or more of the fields is blank");
        return false;
      } else if (!loanAmount.match(regex) || !interestRate.match(regex) || !mortBalance.match(regex) || !pzip.match(regex) || !mInterestRate.match(regex)) {

        if (!loanAmount.match(regex)) {
          alerts.innerHTML += '<p style="color: red;">Sale Price must be a numeric value!</p>';
        }
        if (!interestRate.match(regex)) {
          alerts.innerHTML += '<p style="color: red;">Loan Interest Rate must be a numeric value!</p>';
        }
        if (!mortBalance.match(regex)) {
          alerts.innerHTML += '<p style="color: red;">Mort Balance must be a numeric value!</p>';
        }
        if (!pzip.match(regex)) {
          alerts.innerHTML += '<p style="color: red;">Property Zip Code must be a numeric value!</p>';
        }
        if (!mInterestRate.match(regex)) {
          alerts.innerHTML += '<p style="color: red;">1st Mort Interest % must be a numeric value!</p>';
        }
        return false;
      } else {
        return true;
