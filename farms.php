...
<!-- END SIDE NAV -->
  <!-- end include -->
    ...
    <main>
      <h2>create farms</h2>
      <div class="newFarmsOptionsCont">
        <div class="newFarmsOptions">
          <h3>Polygon/Area</h3>
          <!-- CHANGED LINK * 1/21/20 * -->
          <a href="walkingFarms.php" class="button">Create Here</a>
        </div>
        <div class="newFarmsOptions">
          <h3>Walking Farm</h3>
          <a href="walkingFarms.php" class="button">Start Walking</a>
        </div>
        <div class="newFarmsOptions">
          <!-- <h3>&#x1F3AF; Farm Focus</h3> -->
          <h3>Farm Focus</h3>
          <!-- CHANGED LINK * 1/21/20 * -->
          <a href="farmFocus.php" class="button">Get Focused</a>
        </div>
        <div class="newLiveMap">
        <h3>Live Map</h3>
        <a href="liveMap.php?u=Kyle Jonas&id=293272&z=99507&o=oct" class="button newLiveMapButton">Select Farm from Live Map<a>
        </div>
      </div>

      <div class="myFarmsCont" id="myFarmsCont">
        <div class="myFarms" id="myFarms">
          <h2>My Farms</h2>
          <div class="regFarms" id="regFarms">
            <h3>Farms</h3>
          
              <div class="button" id="show-farms-button">Show All</div>
            <!-- 
              <a href="regFarmView.php">
              <div class="farm" id="farm0">
                <p class="farmName">Creative Farm Name</p>
                <p class="numFarmRecords">1,234</p>
                <p class="farmEditDate">12/06/2019</p>
              </div>
            </a>
            -->
            
          </div>
          <div class="focusedFarms" id="focusedFarms">
            <h3>Focused Farms</h3>
            <!-- added link * 1/22/20 * -->
              <div class="button" id="show-focus-farms-button">Show All</div>
          </div>
<!-- ADDED all farms on map link * 1/24/20 * -->
<a href="allFarmsOnMap.php" class="button" id="farmsOnMap" style="display: none;"> My Farms on Map</a>
  ...
  <script>
    ...
    var memberID = "293272";
    var myFarmsURL = "https://www.titleadvantage.com/mobilemyfarms.asp?u=" + username + "&id=" + memberID;
    //alert(myFarmsURL);

    var showFarmsButton = document.getElementById('show-farms-button');
    var hiddenFarms = document.getElementsByClassName('hide-farm');
    var showFocusFarmsButton = document.getElementById('show-focus-farms-button');
    var hiddenFocusFarms = document.getElementsByClassName('hide-focus-farm');

    showFarmsButton.addEventListener('click', function(){
      for (var i = 0; i < hiddenFarms.length; i++) {
        hiddenFarms[i].style.display = 'block';
      }
      showFarmsButton.style.display = 'none';
    })

    showFocusFarmsButton.addEventListener('click', function(){
      for (var i = 0; i < hiddenFocusFarms.length; i++) {
        hiddenFocusFarms[i].style.display = 'block';
      }
      showFocusFarmsButton.style.display = 'none';
    })
  </script>
</html>