...
<body>
  ...
  <div class="grid-container" id="grid-container">
    ...
    <main>
      ...
      <div class="netAdvantageIcons" id="netAdvantageIcons">
        <form action="quickBuyer.php" method="POST" name="theform" id="netForm" onsubmit="return validateForm()">
          <input type="hidden" name="subit" value="yes">
          <h3>For Buyers</h3>
          <p style="font-size: 13px;">Items with a (*) are required</p>
          <div id="alerts"></div>
          <div>
            <label for="reportName">Report Name</label>
            <input id="reportName" name="reportName" type="text" size="25" maxlength="30"></input>
          </div>

          <div>
            <label for="saleamt">* Sales Price $</label>
            <input type="text" size="10" maxlength="30" name="saleamt" id="saleamt">
          </div>

          <div>
            <label for="lType">Loan Type</label>
            <select NAME="loanType" size="1" id="lType" onchange="loanTypeChange();">
          ...
          <div class="three-wide">
            ...
            <input type="text" size="10" maxlength="20" name="downPayment" id="downpayment" onkeypress="downPaymentChange();" onkeyup="downPaymentChange();">
            <select NAME="dpType" size="1" id="DPType" onchange="downPaymentChange();">
          ...
          <div class="three-wide">
            ...
            <input type="text" size="10" maxlength="12" name="loanamt" id="inputLoanAmount">
            <div style="text-align: right; display: inline-block; width: 36%">
              <button type="button" id="allCashBtn" class="button" style="min-width: 100px; display:inline-block;"> All Cash </button>
            </div>
          </div>

          <div>
            <label for="postal_code">* Property Zip Code</label>
            <input type="text" size="10" maxlength="12" name="pzip" value="" id="postal_code">
          </div>

          <div>
            <label for="inputInterestRate">Loan Interest Rate</label>
            <input type="text" size="10" maxlength="12" name="intRate" value="" id="inputInterestRate">

          </div>

          <div style=" text-align: center;">
            <input type="submit" value=" Continue " id="search" class="button" style="padding-top: 35px;">
            <input type="reset" value=" Cancel - Go Back " onClick="history.back(-1);" class="button" style="padding-top: 35px;">
          </div>
        </form>
  ...
  <script language="JavaScript">
    ...
    var allCashBtn = document.getElementById("allCashBtn");
    var downpayment = document.getElementById("downpayment");


    allCashBtn.addEventListener("click", () => {
        inputLoanAmount.value = "0";
        inputInterestRate.value = "0";
        downpayment.value = "100";
      }) <
      !-- //

    function downPayment() {
      curdownpayment = document.theform.downpayment.value;
      curdownpaymenttype = document.getElementById('downpaymenttypeselector').selectedIndex

      if (curdownpaymenttype < 1) {
        saleamt = document.theform.saleamt.value.replace("$", "");
        loanamt = document.theform.loanamt.value.replace("$", "");
        saleamt = saleamt.replace(/,/g, "");
        loanamt = loanamt.replace(/,/g, "");
        saleamt = parseFloat(saleamt);
        loanamt = parseFloat(loanamt);
        //alert('sale amount:[' + saleamt + '] loan amount:[' + loanamt + ']');
        if (saleamt > 0 && loanamt > 0) {
          downpayment = saleamt - loanamt;
          downpayment = downpayment.toFixed(2).toString();
          document.theform.downpayment.value = downpayment;
          document.getElementById('DPType').selectedIndex = 0;
        }
      }
    }

    function downPaymentChange() {
      curdownpayment = document.theform.downpayment.value.replace("$", "");
      console.log(curdownpayment);
      saleamt = document.theform.saleamt.value.replace("$", "");
      saleamt = saleamt.replace(/,/g, "");
      curdownpayment = curdownpayment.replace(/,/g, "");
      saleamt = parseFloat(saleamt);
      curdownpayment = parseFloat(curdownpayment);
      curdownpaymenttype = document.getElementById('DPType').selectedIndex;
      //so we can change the dropdown when the values goes over 100 to flat
      if (curdownpayment > 100 && curdownpaymenttype == 0) {
        document.getElementById('DPType').selectedIndex = 1;
        curdownpaymenttype = 1;
      }
      //so we can change the dropdown when the values comes down to less then 100 to percent 
      if (curdownpayment < 100 && curdownpaymenttype == 1) {
        document.getElementById('DPType').selectedIndex = 0;
        curdownpaymenttype = 0;
      }

      if (curdownpaymenttype == 0) {
        //percentage
        if (saleamt > 0 && curdownpayment > 0) {
          curdownpayment = (saleamt * curdownpayment) / 100;
          loanamt = saleamt - curdownpayment;
          loanamt = loanamt.toFixed(2).toString();
          document.theform.loanamt.value = " ";
          document.theform.loanamt.value = loanamt;
        }
      } else {
        //flat amount
        if (saleamt > 0 && curdownpayment > 0) {
    ...
    function loanTypeChange() {
      curdownpayment = document.theform.downpayment.value;
      curdownpaymenttype = document.getElementById('DPType').selectedIndex
      curloantype = document.getElementById('lType').selectedIndex
      if ((curdownpayment.length < 1) && (curloantype == 2)) {
        //FHA pre populate
        document.getElementById('DPType').selectedIndex = 1;
        document.theform.downpayment.value = '3.5';
        downPaymentChange();
      }
    ...
    function validateForm() {
      ...
      alerts.innerHTML = "";
      var saleamt = document.forms["theform"]["saleamt"].value;
      saleamt = saleamt.replace("$", "");
      saleamt = saleamt.replace(",", "");
      parseInt(saleamt);
      var downPayment = document.forms["theform"]["downPayment"].value;
      downPayment = downPayment.replace("$", "");
      downPayment = downPayment.replace(",", "");
      downPayment = downPayment.replace("%", "");
      parseInt(downPayment);
      var loanamt = document.forms["theform"]["loanamt"].value;
      loanamt = loanamt.replace("$", "");
      loanamt = loanamt.replace(",", "");
      parseInt(loanamt);
      var pzip = document.forms["theform"]["pzip"].value;
      var intRate = document.forms["theform"]["intRate"].value;
      var regex = /^[0-9.,]+$/;

      if (downPayment == "") {
        downPayment = '0';
        document.forms["theform"]["downPayment"].value = 0;
      }

      if (loanamt == "") {
        loanamt = '0';
        document.forms["theform"]["loanamt"].value = 0;
      }

      if (intRate == "") {
        intRate = '0';
        document.forms["theform"]["intRate"].value = 0;
      }


      if (saleamt == "" || pzip == "") {
        if (saleamt == "") {
          alerts.innerHTML += '<p style="color: red;">Please enter a Sale Price </p>';
        }

        if (pzip == "") {
          alerts.innerHTML += '<p style="color: red;">Please enter a Property Zip</p>';
        }

        console.log("one or more of the fields is blank");
        return false;
      } else if (!saleamt.match(regex) || !downPayment.match(regex) || !loanamt.match(regex) || !pzip.match(regex) || !intRate.match(regex)) {

        if (!saleamt.match(regex)) {
          alerts.innerHTML += '<p style="color: red;">Sale Price must be a numeric value!</p>';
        }
        if (!downPayment.match(regex)) {
          alerts.innerHTML += '<p style="color: red;">Down Payment must be a numeric value!</p>';
        }
        if (!loanamt.match(regex)) {
          alerts.innerHTML += '<p style="color: red;">Loan Amount must be a numeric value!</p>';
        }
        if (!pzip.match(regex)) {
          alerts.innerHTML += '<p style="color: red;">Property Zip Code must be a numeric value!</p>';
        }
        if (!intRate.match(regex)) {
          alerts.innerHTML += '<p style="color: red;">Loan Interest Rate must be a numeric value!</p>';
        }
        return false;
      } else {
        return true;
