...
<head>
  ...
  <script>
    ...
    function citydrop(countyname) {
      loadXMLDoc(countyname);
    }

    function loadXMLDoc() {
      var si = document.getElementById("countyval").selectedIndex;
      var countyname = document.getElementById("countyval").options[si].text;
      var url = 'citylist.asp?county=' + countyname;
      //alert('getting: ' + url);

      if (window.XMLHttpRequest) { // code for IE7+, Firefox, Chrome, Opera, Safari
        xmlhttp = new XMLHttpRequest();
        //alert('getting: ' + url);
        //xmlhttp.onreadystatechange= function(){if(xmlhttp.readyState==4){goUpdate(ddlist)}};
      ...
      //alert('getting: ' + xmlhttp.responseText.indexOf("option"));
      if (xmlhttp.responseText.indexOf("option") > 1) {
        ...
        document.getElementById("citysel").style.display = '';
      } else {
        noCity = true;
        document.getElementById("citysel").style.display = 'none';
        document.getElementById("citylist").style.display = 'none';
        e = document.getElementById("cityval");
        cityval = e.options[e.selectedIndex].text;
        e.remove(cityval);
      }
    }
    ...
    function formcont() {
      ...
      e = document.getElementById("cityval");
      cityval = e.options[e.selectedIndex].text;
      if (cityval.length < 2 || cityval == 'Choose one here') {
        var e = document.getElementById("countyval");
        var countyval = e.options[e.selectedIndex].text;
        citydrop(countyval);
      } else {
...
<body>
  ...
  <div class="grid-container" id="grid-container">
    ...
    <main>
      ...
      <div id="netAdvHistory" class="netAdvHistory less">
        <h2>Net Advantage History</h2>

        <h2>No history was found. Return to Net Advantage to Start a New Net Advantage Report</h2>
        <a href="netadvantage.php" class="button"> Take Me There! </a>


        <div id="dialog" class="modal" style="display:none; text-align: center;">
          <div class="blktext modal-content" style="vertical-align: middle;"> Are you sure you want to delete this record? <BR>
            <P>
              <input type="button" value=" Yes " id="deleteButton" class="button" style="margin: 10px auto;" onclick="deleteEntry();">
              <input type="button" value=" No/Cancel " class="button" style="margin: 10px auto;" onclick="document.getElementById('dialog').style.display = 'none';">
              <input type="hidden" id="deleteURL">
          </div>
        </div>

        <!-- <a href="prop-net0.php">
          <div class="property" id="property0">
            <img src="img/trash-icon.png" alt="Delete">
            <p class="dateAdded">12/06/2019</p>
            <h3>
              1234 Fake Address <br>
              Fake City, CA 123456
            </h3>
            <p class="APN">123-4567-234</p>
          </div>
        </a> -->
  ...
  <script language="JavaScript">
    ...
    function loadReport(reportAddress) {
      window.open(reportAddress, '_blank');
    }

    function deleteEntry() {
      document.getElementById("dialog").style.display = 'none';
      var xhr = new XMLHttpRequest();
      xhr.open("GET", addTemp, true);
      xhr.send();
      console.log(xhr.status);
      console.log(xhr.statusText);
      var removeID = "remove_" + count;
      var propertyRemoveID = "prop-" + propid;
      //var rowtoDelete = document.getElementById(count);
      //rowtoDelete.parentNode.removeChild(rowtoDelete);
      //var rowtoDelete = document.getElementById(removeID);
      //rowtoDelete.parentNode.removeChild(rowtoDelete);
      document.getElementById(propertyRemoveID).style.display = 'none';
      console.log(propertyRemoveID);
      console.log(removeID);
      console.log(xhr.status);
      console.log(xhr.statusText);
    }

    function delRecDialog(e, remID, un, uid, pid, nid, q) {
      e.preventDefault();
      ...
      ntid = nid;
      quick = q;
      addTemp = "https://www.titleadvantage.com/mobilenetadvdelete.asp?u=" + userlog + "&id=" + userID + "&pid=" + propid + "&nid=" + ntid + "&q=" + quick;
      document.getElementById("dialog").style.display = 'block';
      document.getElementById("deleteURL").value = addTemp;

    }

    function deleteEntry() {
      document.getElementById("deleteButton").value = "Deleting Report";

      var deleteURLString = document.getElementById("deleteURL").value;
      console.log("ready to delete with url:" + deleteURLString);

      xmlhttp = new XMLHttpRequest();
      xmlhttp.open("POST", deleteURLString, true);
      xmlhttp.onreadystatechange = function() { //Call a function when the state changes.
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
          var data = JSON.parse(xmlhttp.responseText);
          if (data["Status"] == "Success") {
            var url = "netAdvHistory.php";
